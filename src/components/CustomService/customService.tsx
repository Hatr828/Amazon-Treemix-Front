...
function searchTree(node: SupportNode, query: string, category: string): SearchResult[] {
    ...
    <div>
      <div style={{ display: "flex", justifyContent: "center" }}>
        <div>
          ...
          <span style={{ fontSize: "0.938vw" }}>
            We’ll walk you through fixing most things here, or connect you to someone if you need
            more help.
          </span>
          <div className="div-description-text">
            What would you like help with today? You can quickly take care of most things here, or
          ...
          <div className="div-for-icon-service">
            {categories.map((category) => (
              <Link
                key={category.id}
                href={`/customService/${category.id}`}
          ...
          <div className="div-description-text">Search the help library</div>
          <div
            style={{
              display: "flex",
              position: "relative",
                ...
                {results.slice(0, 3).map((item) => (
                  <div key={item.id} className="search-item">
                    <div
                      ...
                      onClick={() => router.push(`/customService/${item.category}?node=${item.id}`)}
                    >
                      {item.title}
                    </div>
                    {item.answer && (
                      <div style={{ fontSize: "1vw", opacity: 0.7 }}>{item.answer}</div>
                    )}
                  </div>
                ))}
              </div>
